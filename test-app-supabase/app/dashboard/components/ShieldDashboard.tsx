...
const NAV_ITEMS: Array<{ id: Page; label: string; icon: string }> = [
  ...
  { id: 'blocked-ips', label: 'Blocked IPs', icon: 'X' },
...
function OverviewPage({ stats, loading }: { stats: IncidentStats | null; loading: boolean }) {
  if (loading) return <LoadingState />
  if (!stats) return <EmptyState text="No data available" />

  const categoryColors: Record<ThreatCategory, string> = {
    sqli: '#ff3366', xss: '#ff00ff', lfi: '#00d4ff',
    ssrf: '#ffaa00', 'path-traversal': '#9966ff',
  }

  const maxCategoryCount = Math.max(
    ...(Object.values(stats.categoryBreakdown) as number[]), 1
  )
  ...
  return (
    <>
      <div className="shield-page-header">
        ...
        <p className="shield-page-subtitle">System Overview // Real-Time Threat Intelligence</p>
      </div>
      ...
      <div className="shield-stats-grid">
        ...
        <div className="shield-stat-card">
          ...
          <div className="shield-stat-value shield-stat-value--danger">{formatNumber(stats.totalBlockedIPs)}</div>
        </div>
        <div className="shield-stat-card">
        ...
        <div className="shield-chart">
          ...
          <div className="shield-bar-chart">
            {stats.hourlyTimeline.length > 0 ? (
              stats.hourlyTimeline.map((point, i) => {
                const maxCount = Math.max(...stats.hourlyTimeline.map((p) => p.count), 1)
                const height = (point.count / maxCount) * 100
                return (
                  <div key={i} className="shield-bar" style={{ height: `${Math.max(height, 2)}%` }}
            ...
            {(Object.entries(stats.categoryBreakdown) as [ThreatCategory, number][]).map(([cat, count]) => (
              <div key={cat} className="shield-category-bar">
                ...
                <div className="shield-category-bar-track">
                  <div className="shield-category-bar-fill"
                    style={{ width: `${(count / maxCategoryCount) * 100}%`, background: categoryColors[cat] || '#00ff88' }} />
                </div>
        ...
        <div className="shield-chart">
          ...
          <div className="shield-table-wrapper" style={{ clipPath: 'none' }}>
            <table className="shield-table">
              <thead><tr><th>IP Address</th><th>Attacks</th><th>Last Category</th></tr></thead>
              <tbody>
                {stats.topAttackerIPs.map((attacker, i) => (
                  <tr key={i}>
                    <td style={{ fontFamily: 'var(--shield-font-body)', color: 'var(--shield-accent)' }}>{attacker.ip}</td>
                    <td>{attacker.count}</td>
                    <td><CategoryBadge category={attacker.lastCategory} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem', alignItems: 'center' }}>
        <select className="shield-select" style={{ width: '200px' }}
        ...
        <button className="shield-btn shield-btn--ghost" onClick={() => fetchIncidents()}>Refresh</button>
      </div>

      {loading ? <LoadingState /> : !incidents || incidents.data.length === 0 ? (
        <EmptyState text="No incidents detected" />
      ) : (
        <>
          <div className="shield-table-wrapper">
            <table className="shield-table">
              <thead><tr><th>Timestamp</th><th>Source IP</th><th>Category</th><th>Path</th><th>Confidence</th><th>Action</th></tr></thead>
              <tbody>
                {incidents.data.map((inc, i) => (
                  <tr key={inc.id || i}>
                    <td style={{ whiteSpace: 'nowrap', color: 'var(--shield-muted-fg)', fontSize: '0.7rem' }}>{formatTimestamp(inc.timestamp)}</td>
                    <td style={{ fontFamily: 'var(--shield-font-body)', color: 'var(--shield-accent)' }}>{inc.sourceIP}</td>
                    <td><CategoryBadge category={inc.attackCategory} /></td>
                    <td style={{ maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{inc.requestPath}</td>
                    <td><ConfidenceBar value={inc.confidence} /></td>
                    <td><span className={`shield-badge shield-badge--${inc.action}`}>{inc.action}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="shield-pagination">
            <div className="shield-pagination-info">Page {incidents.page} of {Math.ceil(incidents.total / incidents.limit)} // {incidents.total} total</div>
            <div className="shield-pagination-controls">
              <button className="shield-btn shield-btn--ghost shield-btn--sm" disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>Prev</button>
              <button className="shield-btn shield-btn--ghost shield-btn--sm" disabled={!incidents.hasMore} onClick={() => setPage((p) => p + 1)}>Next</button>
            </div>
          </div>
        </>
      )}
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem' }}>
        <div className="shield-input-wrapper" style={{ flex: 1, maxWidth: '300px' }}>
          ...
          <input type="text" className="shield-input" placeholder="Enter IP to block..."
            value={blockInput} onChange={(e) => setBlockInput(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleBlock()} />
        </div>
        <button className="shield-btn shield-btn--danger" onClick={handleBlock}>Block IP</button>
      </div>

      {loading ? <LoadingState /> : !blockedIPs || blockedIPs.data.length === 0 ? (
        <EmptyState text="No IPs currently blocked" />
      ) : (
        <div className="shield-table-wrapper">
          <table className="shield-table">
            <thead><tr><th>IP Address</th><th>Reason</th><th>Strikes</th><th>Blocked At</th><th>Expires</th><th>Last Attack</th><th>Actions</th></tr></thead>
            <tbody>
              {blockedIPs.data.map((ip, i) => (
                <tr key={ip.id || i}>
                  <td style={{ fontFamily: 'var(--shield-font-body)', color: 'var(--shield-destructive)' }}>
                    <span className="shield-status-dot shield-status-dot--danger" />{ip.ipAddress}
                  </td>
                  <td>{ip.reason}</td>
                  <td style={{ color: 'var(--shield-warning)' }}>{ip.strikeCount}</td>
                  <td style={{ fontSize: '0.7rem', color: 'var(--shield-muted-fg)' }}>{formatTimestamp(ip.blockedAt)}</td>
                  <td style={{ fontSize: '0.7rem', color: 'var(--shield-muted-fg)' }}>{ip.expiresAt ? formatTimestamp(ip.expiresAt) : 'Permanent'}</td>
                  <td><CategoryBadge category={ip.lastAttackCategory} /></td>
                  <td><button className="shield-btn shield-btn--ghost shield-btn--sm" onClick={() => handleUnblock(ip.ipAddress)}>Unblock</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function ReportsPage({ apiBase }: { apiBase: string }) {
  ...
  return (
    <>
      ...
      {selectedReport ? (
        <div>
          <button className="shield-btn shield-btn--ghost" onClick={() => setSelectedReport(null)}
            style={{ marginBottom: '1rem' }}>Back to Reports</button>
          <ReportDetail report={selectedReport} />
        </div>
      ) : loading ? <LoadingState /> : !reports || reports.data.length === 0 ? (
        <EmptyState text="No reports generated yet" />
      ) : (
        <div style={{ display: 'grid', gap: '1rem' }}>
          {reports.data.map((report, i) => (
            <div key={report.id || i} className="shield-card shield-card--hoverable" style={{ cursor: 'pointer' }}
              onClick={() => report.id && viewReport(report.id)}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div className="shield-card-title">Report -- {formatTimestamp(report.createdAt)}</div>
...
function ReportDetail({ report }: { report: AIReport }) {
  ...
  const sections = [
    { title: 'Pattern Analysis', data: parseJSON(report.patternAnalysis) },
    { title: 'Trend Analysis', data: parseJSON(report.trendAnalysis) },
    { title: 'Risk Assessment', data: parseJSON(report.riskAssessment) },
    { title: 'Recommendations', data: parseJSON(report.recommendations) },
  ]
    ...
    <div style={{ display: 'grid', gap: '1rem' }}>
      <div className="shield-card shield-card--holographic">
        ...
        <div style={{ fontSize: '0.7rem', color: 'var(--shield-muted-fg)', display: 'flex', gap: '1.5rem' }}>
          <span>{formatTimestamp(report.dateRangeStart)} to {formatTimestamp(report.dateRangeEnd)}</span>
          <span>{report.incidentCount} incidents</span>
          <span>{report.modelUsed}</span>
        </div>
        <div className="shield-divider" />
        <div style={{ fontSize: '0.85rem', lineHeight: 1.7 }}>{report.executiveSummary}</div>
      </div>

      {sections.map(({ title, data }) => data && (
        <div key={title} className="shield-card">
          <div className="shield-card-title">{title}</div>
    ...
    <div style={{ fontSize: '0.8rem', lineHeight: 1.7 }}>
      {Object.entries(data).map(([key, value]) => (
        <div key={key} style={{ marginBottom: '0.75rem' }}>
          <div style={{
            fontFamily: 'var(--shield-font-accent)', fontSize: '0.65rem',
            textTransform: 'uppercase' as const, letterSpacing: '1.5px',
            color: 'var(--shield-accent)', marginBottom: '0.25rem',
          }}>
  ...
  return (
    <>
      <div className="shield-page-header">
        ...
        <p className="shield-page-subtitle">System Configuration // Supabase Backend</p>
      </div>
      ...
      <div style={{ display: 'grid', gap: '1rem', maxWidth: '600px' }}>
        <div className="shield-card">
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            <SettingField label="Confidence Threshold" value={settings['confidenceThreshold'] || '0.7'}
              onChange={(v) => setSettings({ ...settings, confidenceThreshold: v })} />
            <SettingField label="Max Strikes" value={settings['maxStrikes'] || '3'}
              onChange={(v) => setSettings({ ...settings, maxStrikes: v })} />
            <SettingField label="Block Duration (seconds)" value={settings['blockDuration'] || '86400'}
              onChange={(v) => setSettings({ ...settings, blockDuration: v })} />
          </div>
        </div>

        <button className="shield-btn shield-btn--filled" onClick={saveSettings} disabled={saving} style={{ justifySelf: 'start' }}>
...
function SettingField({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return (
    <div>
      <label style={{
        display: 'block', fontFamily: 'var(--shield-font-accent)', fontSize: '0.65rem',
        textTransform: 'uppercase' as const, letterSpacing: '1.5px',
        color: 'var(--shield-muted-fg)', marginBottom: '0.25rem',
      }}>{label}</label>
      <div className="shield-input-wrapper">
        <span className="shield-input-prefix">&gt;</span>
        <input type="text" className="shield-input" value={value} onChange={(e) => onChange(e.target.value)} />
      </div>
    </div>
  )
}
...
function CategoryBadge({ category }: { category: ThreatCategory }) {
  return <span className={`shield-badge shield-badge--${category}`}>{CATEGORY_LABELS[category] || category}</span>
}

function ThreatLevelBadge({ level }: { level: string }) {
  return <span className={`shield-badge shield-badge--${level}`}>{level}</span>
}
...
function ConfidenceBar({ value }: { value: number }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      <div style={{ width: '60px', height: '4px', background: 'var(--shield-muted)', overflow: 'hidden' }}>
        <div style={{ width: `${pct}%`, height: '100%', background: color, transition: 'width 300ms' }} />
      </div>
      <span style={{ fontSize: '0.7rem', color, fontFamily: 'var(--shield-font-body)' }}>{pct.toFixed(0)}%</span>
    </div>
  )
}
  ...
  return (
    <div className="shield-loading">
      <div className="shield-loading-dots">
        <div className="shield-loading-dot" /><div className="shield-loading-dot" /><div className="shield-loading-dot" />
      </div>
    </div>
  )
}
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="shield-empty">
      <div className="shield-empty-icon">&gt;_</div>
